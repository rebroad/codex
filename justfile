...
[unix]
test *args:
    @cd "{{ cargo_working_directory }}" && {{ android_cargo_setup }} {{ rusty_v8_setup }} {{ openssl_setup }} CARGO_TARGET_DIR={{ cargo_target_dir }} cargo build --locked -p codex-cli -p codex-code-mode-host
    @cd "{{ cargo_working_directory }}" && {{ android_cargo_setup }} {{ rusty_v8_setup }} {{ openssl_setup }} CARGO_TARGET_DIR={{ cargo_target_dir }} cargo build --locked -p codex-rmcp-client --bin test_stdio_server
    @cd "{{ cargo_working_directory }}" && {{ android_cargo_setup }} {{ rusty_v8_setup }} {{ openssl_setup }} CARGO_TARGET_DIR={{ cargo_target_dir }}; RUST_MIN_STACK={{ rust_min_stack }}; export CARGO_TARGET_DIR RUST_MIN_STACK RUSTY_V8_ARCHIVE RUSTY_V8_SRC_BINDING_PATH; if command -v cargo-nextest >/dev/null 2>&1 || test "$(rustc -vV | sed -n 's/^host: //p')" != aarch64-linux-android; then NEXTEST_PROFILE=local cargo nextest run --locked --no-fail-fast "$@"; else echo "cargo-nextest is unavailable on Android; using cargo test" >&2; cargo test --locked "$@"; fi
