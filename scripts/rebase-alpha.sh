...
git rebase --onto "${upstream_alpha_commit}" "${fork_point_commit}" alpha
echo "Updating ${FORK_POINT_BRANCH} to ${UPSTREAM_ALPHA_BRANCH}..."
git update-ref "${fork_point_ref}" "${upstream_alpha_commit}" "${fork_point_commit}"
