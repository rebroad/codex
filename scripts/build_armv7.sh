...
resolve_build_commit_short() {
  ...
  if [[ -n "${CODEX_BUILD_COMMIT_SHORT:-}" ]] && [[ "${CODEX_BUILD_COMMIT_SHORT}" =~ ^[0-9a-f]{7,40}$ ]]; then
    echo "${CODEX_BUILD_COMMIT_SHORT:0:10}"
    return 0
  fi

  if command -v git >/dev/null 2>&1; then
    if commit="$(git -C "${REPO_DIR}" rev-parse --short=10 HEAD 2>/dev/null)"; then
      echo "${commit}"
      return 0
    fi
    ...
    if commit="$(git -C "${candidate}" rev-parse --short=10 HEAD 2>/dev/null)"; then
        echo "${commit}"
        return 0
      fi
    done
...
run_in_docker_buster() {
  ...
  if [[ "${container_cargo_target_dir}" != /work/codex/* ]]; then
    container_cargo_target_dir="/work/codex/build/armv7-${PROFILE}"
  fi
...
with bin_path.open("r+b") as f:
    ...
    try:
        ...
        if count == 0:
            pattern = re.compile(re.escape(base_version) + rb"-(?:[0-9a-f]{10,12}\+?)-\d{12}")
...
# sccache cannot handle the incremental cross-compilation mode used by this
# builder; disable the wrapper explicitly so host Cargo configuration cannot
# turn this into a late rustc-probe failure.
cargo_env=(RUSTUP_DISABLE_SELF_UPDATE=1 CARGO_INCREMENTAL=0 RUSTC_WRAPPER=)
if [[ -n "${BUILD_JOBS}" ]]; then
