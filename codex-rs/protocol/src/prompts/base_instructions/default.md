...
- Emit function calls to run terminal commands and apply patches. Depending on how this specific run is configured, you can request that these function calls be escalated to the user for approval before running. To apply a patch with escalated permissions, run the `apply_patch` command through the shell with `sandbox_permissions: "require_escalated"`; the dedicated `apply_patch` tool does not take a permissions parameter. More on this in the "Sandbox and approvals" section.
