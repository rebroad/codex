    ...
    for ancestor in cwd.ancestors() {
        for marker in project_root_markers {
            ...
            let is_synthetic_temp_codex_marker = marker == ".codex"
                ...
                && marker_path.is_dir();
            // Container executors can mount their task repository at the filesystem root.
            if (marker == ".git" && (is_temp_marker_root || ancestor.as_path() == Path::new("/")))
...
async fn find_git_checkout_root(
    ...
    for dir in base.ancestors() {
        ...
        if is_synthetic_temp_marker_root(dir.as_path()) || dir.as_path() == Path::new("/") {
            continue;
        }
