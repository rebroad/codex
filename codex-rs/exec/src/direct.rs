...
pub(crate) async fn run(prompt_text: String, config: &Config, json_mode: bool) -> Result<()> {
    ...
    if let Some(base_instructions) = config.base_instructions.clone() {
        prompt.base_instructions.text = base_instructions;
    } else if !config.bare_prompt {
        prompt.base_instructions.text = model_info.get_model_instructions(config.personality);
    }
...
fn build_inputs(developer_instructions: Option<&str>, prompt_text: &str) -> Vec<ResponseItem> {
