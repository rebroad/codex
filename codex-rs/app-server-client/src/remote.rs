    ...
    async fn connect_with_policy(
        ...
        match args.endpoint {
            ...
            RemoteAppServerEndpoint::UnixSocket { socket_path } => {
                let (endpoint, stream) =
                    connect_unix_socket_endpoint(socket_path.clone(), peer_policy).await?;
                let reconnect = Box::new(move || {
                    let socket_path = socket_path.clone();
                    Box::pin(connect_unix_socket_endpoint(socket_path, peer_policy))
                        as Pin<
                            Box<
