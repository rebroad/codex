...
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/rebroad/codex/discussions/7782";
...
impl ModelProviderInfoOverrides {
    pub fn apply_to(&self, provider: &mut ModelProviderInfo) {
        ...
        if let Some(value) = &self.model_catalog_url {
            provider.model_catalog_url = Some(value.clone());
        }
        ...
        if let Some(value) = &self.gateway_oauth {
            provider.gateway_oauth = Some(value.clone());
        }
...
impl From<ModelProviderInfo> for ModelProviderInfoOverrides {
    fn from(provider: ModelProviderInfo) -> Self {
        Self {
            ...
            auth: provider.auth,
            gateway_oauth: provider.gateway_oauth,
            aws: provider.aws,
            wire_api: Some(provider.wire_api),
            query_params: provider.query_params.map(|params| {
                params
                    .into_iter()
                    .map(|(key, value)| (key, value.into_inner()))
                    .collect()
            }),
            http_headers: provider.http_headers.map(|headers| {
                headers
                    .into_iter()
                    .map(|(key, value)| (key, value.into_inner()))
                    .collect()
            ...
            supports_websockets: Some(provider.supports_websockets),
            supports_standalone_web_search: Some(provider.supports_standalone_web_search),
        }
    }
}
