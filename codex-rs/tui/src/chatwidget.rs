...
use crate::status::fetch_rate_limits;
...
impl ChatWidget {
    ...
    fn prefetch_rate_limits(&mut self) {
        ...
        let rate_limit_adjustments = RateLimitAdjustments {
            ...
            short_used_percent_max: self.config.rate_limit_short_used_percent_max,
            weekly_reset_at_seconds: self.config.rate_limit_weekly_reset_at_offset_seconds,
            weekly_used_percent_max: self.config.rate_limit_weekly_used_percent_max,
        };
        let app_event_tx = self.app_event_tx.clone();
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                {
                    for snapshot in
                        fetch_rate_limits(base_url.clone(), auth, rate_limit_adjustments).await
                    {
