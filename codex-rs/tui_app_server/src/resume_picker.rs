...
pub async fn run_fork_prompt_picker_for_turns(
    tui: &mut Tui,
    turns: &[AppServerTurn],
) -> Result<Option<usize>> {
    let prompt_points = load_fork_prompt_points_from_turns(turns);
    run_fork_prompt_picker_for_points(tui, prompt_points).await
}

async fn run_fork_prompt_picker_for_points(
    tui: &mut Tui,
    prompt_points: Vec<ForkPromptPoint>,
) -> Result<Option<usize>> {
...
fn load_fork_prompt_points_from_turns(turns: &[AppServerTurn]) -> Vec<ForkPromptPoint> {
    turns
        .iter()
        .filter_map(|turn| {
            turn.items.iter().find_map(|item| match item {
                AppServerThreadItem::UserMessage { content, .. } => {
                    Some(text_from_user_inputs(content.as_slice()))
                }
                _ => None,
            })
        })
        .enumerate()
        .map(|(nth_user_message, message)| ForkPromptPoint {
            nth_user_message,
            message,
        })
        .collect()
...
fn text_from_user_inputs(content: &[AppServerUserInput]) -> String {
    content
        .iter()
        .filter_map(|input| match input {
            AppServerUserInput::Text { text, .. } => Some(text.trim()),
            AppServerUserInput::Image { .. }
            | AppServerUserInput::LocalImage { .. }
            | AppServerUserInput::Skill { .. }
            | AppServerUserInput::Mention { .. } => None,
        })
...
impl PickerState {
    ...
    async fn update_prompt_counts(&mut self) {
        ...
        for row in self.all_rows.iter_mut() {
            ...
            let cached = self.prompt_count_cache.get(path).copied().flatten();
    ...
    async fn update_context_used_percent(&mut self) {
        ...
        for row in self.all_rows.iter_mut() {
            let cached = row
                .path
                .as_ref()
                .and_then(|path| self.context_used_percent_cache.get(path))
                .copied()
                .flatten();
