...
- Always run `gh` CLI commands in an escalated shell.
...
- For Cargo commands that support it, prefer `--locked` to avoid incidental lockfile rewrites (for example `cargo build --locked`, `cargo clippy --locked`, `cargo test --locked`).
- Prefer end-to-end build verification with `./scripts/rebuild_codex.sh` (run from the designated build tree) over localized `cargo check` commands.
...
- Prefer ratatui `Stylize` helpers and simple conversions (`"text".dim()`, `"text".into()`, `vec![...].into()`), and avoid style churn when equivalent forms are already used in-file.
- For wrapping, use existing helpers (`textwrap::wrap`, `tui/src/wrapping.rs`, `prefix_lines`) instead of custom wrapping/prefix logic.
- Avoid growing high-touch TUI files; extract new functionality instead.
- UI-visible TUI changes require `insta` coverage; do not hand-edit `.snap` files. Generate and accept via `cargo insta` workflow.
- In tests, prefer `pretty_assertions::assert_eq` and whole-object equality assertions when practical.
- For spawning first-party binaries/resources in tests, prefer `codex_utils_cargo_bin::cargo_bin` and `codex_utils_cargo_bin::find_resource!`.
- For core integration tests, prefer `core_test_support::responses` helpers and assert captured `/responses` requests via `ResponseMock`.
- App-server work is v2-only. Keep payload naming consistent (`*Params`, `*Response`, `*Notification`) and RPC methods as `<resource>/<method>` with singular resources.
- Keep app-server wire names camelCase (except config RPC payloads), keep serde/ts renames aligned, and use explicit tagged unions (`type`) for discriminated unions.
- App-server `*Params` optional fields should use `Option<T>` + `#[ts(optional = nullable)]`; avoid `skip_serializing_if` on v2 payload fields except intentionally param-less `params: Option<()>`.
- New app-server list methods should use cursor pagination (`cursor`, `limit`, `data`, `next_cursor`); update docs/schema fixtures when API shapes change.
